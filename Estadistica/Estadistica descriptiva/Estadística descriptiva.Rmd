...
```{r}
ggplot(data, aes(x = cyl, y = hwy))+ geom_point() # Gráfico de dispersión
```

Prueba a cambiar la capa `geom_point()` por `geom_boxplot()`:

```{r}
ggplot(data, aes(x = cyl, y = hwy))+ geom_boxplot() # Capa en forma de boxplots
```
...
# Gráfica
ggplot(data, aes(x = cyl, y = hwy))+ geom_boxplot()
```
