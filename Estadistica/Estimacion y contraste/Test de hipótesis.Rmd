...
ggplot(data, aes(x = ??????, y = after_stat(density)))+  
	geom_histogram(bins = ??????, fill = "gray", colour = "black")+
  geom_density(lwd = 1)
```
...
test = t.test(data$weight_loss_Kg, mu = ????, conf.level = 0.95)
...
```{r}
pwr.t.test(n=?????, d = eff$Cohens_d, sig.level = ?????, type = "?????")
```
...
```{r}
# Realizamos un T-Test
test = t.test(data$weight_loss_Kg, mu = ?????, conf.level = 0.95, alternative = "?????")
test
...
#Potencia
pwr.t.test(n=?????, d = eff$Cohens_d, sig.level = 0.05, type = "?????", alternative = "?????")
```
...
```{r}
test = varTest(data$iq, alternative = "?????", sigma.squared = ?????)
test
```
...
# Suponemos H0 falsa y consideramos la desviación real *cercana* (ley de los grandes números) a la estimada puntualmente.
sd_real = ?????
...
data = read_delim("howell1.csv", delim = ";", escape_double = FALSE, trim_ws = TRUE, show_col_types = F)
```
...
```{r}
pwr.t2n.test(n1 = ?????,         # Tamaño muestra 1
             n2 = ?????,         # Tamaño muestra 2
             d = ?????, # Tamaño del efecto
             sig.level = 0.99
)
...
test = t.test(x = antes, y = despues, conf.level = 0.95, paired = ?????, alternative = "?????")
test
```
...
Calcula la potencia.
```{r}
pwr.t.test(n = 12, d = -0.55, sig.level = 0.05, type = "?????", alternative = "?????")
```

¿Cuántos sujetos necesitamos para mejorar la potencia al 95\%?

```{r}
pwr.t.test(power = 0.95, d = -0.55, sig.level = 0.05, type = "?????", alternative = "?????")
```
