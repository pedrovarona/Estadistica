...
```{r}
crypto = crypto_large[, c("unix_time", "candlestick_low", "candlestick_high", "crypto")]
colnames(crypto) = c("unix_time", "low", "high", "crypto") 
```
...
```{r}
crypto$date = as.POSIXct(crypto$unix_time, origin = "1970-01-01")
```
...
```{r}
btc <- crypto[crypto$crypto == "BTC", ]
eth <- crypto[crypto$crypto == "ETH", ]
plot(btc$date, btc$high, type = "l", ylim = c(0, max(btc$high)))
lines(eth$date, eth$high, col = 2)
```
...
```{r}
counter = 1
crypto_names = unique(crypto$crypto)
for (cr in crypto_names) {
    tmp = crypto[crypto$crypto == cr, ]
    if (counter == 1) {
        plot(tmp$date, log(tmp$high), type = "l", ylim = c(-5, 20), 
             xlab = "Date", ylab = "USD")
    } else {
        lines(tmp$date, log(tmp$high), col = counter)
    }
    counter = counter + 1
}
legend("topright", crypto_names, col = 1:length(crypto_names), lty = 1, 
       bty = "n")
```
...
```{r}
calculate_drop = function(df, crypto_name) {
    crypto_data = df[df$crypto == crypto_name,] 
    (max(crypto_data$high) - min(crypto_data$low)) / max(crypto_data$high)
}

result = sapply(crypto_names, calculate_drop, df = crypto)
print(result)
```
